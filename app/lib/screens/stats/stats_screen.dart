...
class StatsOverview extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold( // Wrap with Scaffold to add AppBar and back button
      appBar: AppBar(
        title: Text("Billing Statistics", style: textTheme.titleLarge?.copyWith(color: colorScheme.onPrimary)), // Themed title
      ...
      body: BlocBuilder<BillingCubit, BillingState>(
        builder: (context, state) {
          if (state is BillingLoading) {
            return Center(child: CircularProgressIndicator(color: colorScheme.primary)); // Themed loading indicator
          } else if (state is BillingError) {
            return Center(
              child: Padding(
                padding: const EdgeInsets.all(24.0), // Added padding for error state
                child: Column(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    Icon(Icons.error_outline, color: colorScheme.error, size: 60), // Themed error icon, larger size
                    const SizedBox(height: 24),
                    Text(
                      state.message,
                      textAlign: TextAlign.center,
                      style: textTheme.bodyLarge?.copyWith(color: colorScheme.error), // Themed error text
                    ),
                    const SizedBox(height: 16),
                    ElevatedButton( // Styled Retry button
                      onPressed: () {
                      ...
                      child: Text('Retry', style: textTheme.labelLarge?.copyWith(color: colorScheme.onPrimary)), // Themed button text
                    ),
                  ],
                ),
              ),
            ...
            return _buildBillingInfoCard(state.billingInfo, context); // Pass context for theming
          } else {
              context.read<BillingCubit>().fetchBillingInfo();
            ...
            return Center(
              child: Padding(
                ...
                child: Text(
                  "Tap to load stats",
                  style: textTheme.bodyMedium?.copyWith(color: colorScheme.onSurface.withOpacity(0.6)), // Muted placeholder text
                  textAlign: TextAlign.center,
                ),
              ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              "Billing Overview",
              style: textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w600, color: colorScheme.onSurface), // Themed title, slightly bolder
            ),
            ...
            _buildStatRow(
              ...
              label: "Usage",
              value: "${billingInfo.usage.toStringAsFixed(2)} / ${billingInfo.limit.toStringAsFixed(2)}", // Format decimals
              unit: "credits", // Assuming 'credits' as unit, adjust as needed.
              isPercentage: billingInfo.limit > 0, // Show progress bar only if limit is set
            ...
            _buildStatRow(
              ...
              label: "Max Requests",
              value: billingInfo.maxRequests.toInt().toString(), // Assuming requests are integers
              unit: "requests",
            ),
        ...
        Expanded(
          child: Column(
            ...
            children: [
              Text(
                label,
                style: textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w500, color: colorScheme.onSurface), // Themed label text
              ),
              Row(
                crossAxisAlignment: CrossAxisAlignment.baseline,
                textBaseline: TextBaseline.alphabetic, // Align baseline for value and unit
                children: [
                  Text(
                    value,
               //     style: textTheme.headline6?.copyWith(fontWeight: FontWeight.bold, color: colorScheme.onSurface), // Themed and larger value text
                  ...
                  if (unit != null)
                    Padding(
                      ...
                      child: Text(unit, style: textTheme.bodyMedium?.copyWith(color: colorScheme.onSurface.withOpacity(0.7))), // Themed unit text, muted
                    ),
                ],
              ),
              if (isPercentage && currentValue != null && maxValue != null && maxValue > 0)
                Padding(
                  padding: const EdgeInsets.only(top: 8.0), // Add some spacing above progress bar
                  child: LinearProgressIndicator( // Styled Progress bar
                    value: currentValue / maxValue,
                    backgroundColor: colorScheme.surfaceVariant, // Themed background color
                    color: colorScheme.primary, // Themed progress color
                    minHeight: 6, // Make progress bar a bit thicker
                    borderRadius: BorderRadius.circular(3), // Rounded progress bar
                  ),
                ),
            ],
          ),
